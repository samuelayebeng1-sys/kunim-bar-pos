...
export default function AdminScreen() {
  ...
  async function loadDashboard() {
    ...
    wk.forEach(x => wd[x] = 0);
    ws.docs.forEach(doc => { const o = doc.data() as Order; if (wd[o.date] !== undefined) wd[o.date] += o.total || 0; });
    setWeekData(wk.map(x => ({ date: x.slice(5), revenue: parseFloat((wd[x] || 0).toFixed(2)) })));
  }

  async function loadAdminReports(period: 'today' | 'week' | 'month') {
    ...
    const snap = await getDocs(query(collection(db, 'orders'), where('date', '>=', from), where('date', '<=', today)));
  ...
  const repTopItems = Object.entries(repByItem).sort((a, b) => b[1] - a[1]).slice(0, 5);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px' }}>
        ...
        {tab === 'reports' && (
          <>
            <div style={{ fontFamily: 'Syne', fontSize: '18px', fontWeight: 700, marginBottom: '14px' }}>Reports</div>
            <div style={{ display: 'flex', gap: '6px', marginBottom: '14px', flexWrap: 'wrap' }}>
              {(['today', 'week', 'month'] as const).map(p => (
                <button key={p} onClick={() => loadAdminReports(p)} style={{ padding: '7px 14px', borderRadius: '8px', fontSize: '12px', fontWeight: 700, border: `1px solid ${reportPeriod === p ? 'var(--red)' : 'var(--border)'}`, background: reportPeriod === p ? 'var(--red)' : 'var(--bg3)', color: reportPeriod === p ? '#fff' : 'var(--text2)', fontFamily: 'Syne', cursor: 'pointer' }}>
